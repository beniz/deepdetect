...
namespace dd
{
	...
	if (!test)
	...
	else
	  {
	    if ((hcit=hcorresp_r.find(cls))!=hcorresp_r.end())
	      cl = (*hcit).second;
	    else _logger->warn("unknown class {} in test set", cls);
	  }
            ...
            if (folder)
              {
                ...
                if (_uris.size() > 1)
                  {
		    read_image_folder(test_lfiles, hcorresp,
                                      hcorresp_r, _uris.at(1), true);
                  }
  ...
  template <typename T>
  void ImgTorchInputFileConn::shuffle_dataset(std::vector<std::pair<std::string, T>> &lfiles)
  {
    std::mt19937 g;
    if (_seed >= 0)
      g = std::mt19937(_seed);
