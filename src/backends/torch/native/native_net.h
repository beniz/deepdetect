...
namespace dd
{
  ...
  public:
    ...
    /// Customized to retrieve copies from every GPU device after reset() has been called once.
    /// When a deep module copy already exists, only the new weights are copied if the
    /// target device differs from the source.
  ...
  private:
    ...
    mutable std::unordered_map<std::string, std::shared_ptr<Derived>> _copies; /**< deep module copies holder, key is device string. */
  };
