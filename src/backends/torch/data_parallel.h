...
namespace parallel {
 ...
 template <typename ModuleType>
   std::vector<ModuleType> _replicas; /**< holder of module replica pointers. */

 /**
  * \brief Customized torch data_parallel to avoid deep module replication in two situations:
  *      - When the weights to be copied are already on the same device as target.
  *      - When successive iterations do accumulate over the loss and no weight update is required. This allows using `iter_size` to spend more time on every GPU, accumulating the loss over large batches.
  */
 template <typename ModuleType>
Tensor dd_data_parallel(
    ModuleType module,
    Tensor input,
  ...
  if (replicate)
    _replicas<ModuleType> = torch::nn::parallel::replicate(module, *devices);

  auto outputs = torch::nn::parallel::parallel_apply(_replicas<ModuleType>, scattered_inputs, *devices);
